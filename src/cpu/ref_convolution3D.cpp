...
void _ref_convolution3D_fwd_t<with_relu, src_type, wei_type, dst_type, acc_type>
    ...
    auto ker = [=](acc_data_t &d, int g, int mb, int oc, int oh, int ow, int od) {
            ...
            for (int kh = 0; kh < KH; ++kh) {
                for (int kw = 0; kw < KW; ++kw) {
                    for (int kd = 0; kd < KD; ++kd) {
                        ...
                        d += (acc_data_t)src[src_d.off(mb, g*IC + ic, ih, iw, id)]
                            * (with_groups
                ...
                for (int oh = 0; oh < OH; ++oh) {
                    for (int ow = 0; ow < OW; ++ow) {
                        for (int od = 0; od < OD; ++od) {
                            ...
                            ker(a, g, mb, oc, oh, ow, od);
                            if (with_relu && a < (acc_data_t)0)
                                a = (acc_data_t)((float)a * nslope);
                            dst[dst_d.off(mb, g*OC + oc, oh, ow, od)]
