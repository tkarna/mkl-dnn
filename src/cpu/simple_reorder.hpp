...
#       pragma omp parallel for collapse(4) schedule(static)
        for (int n = 0; n < dims[0]; ++n) {
            for (int d = 0; d < dims[2]; ++d) {
                for (int h = 0; h < dims[3]; ++h) {
                    for (int w = 0; w < dims[4]; ++w) {
                        auto i = &input[input_d.blk_off(n, 0, d, h, w)];
                        auto o = &output[output_d.blk_off(n, 0, d, h, w)];
                        ker(i, o);
                    }
                }
            }
...
//                             || fmt_o == gIOdhw16o16i))
//         || (fmt_i == oidhw && (fmt_o == OIdhw8o8i || fmt_o == OIdhw16o16i
//                             || fmt_o == IOdhw16o16i))
...
//             (fmt_o == OIdhw8o8i || fmt_o == gOIdhw8o8i) ? 8 : 16;
        constexpr int blksize = 16;
...
#               pragma omp simd collapse(2)
                for (int ic = 0; ic < blksize; ++ic) {
                    for (int oc = 0; oc < blksize; ++oc) {
                        if (order_keep) {
                            o[ic * blksize + oc] =
                                data_t<type_o>(i[oc + ic * _dhwio_st[1]]);
                        } else {
                            o[oc + ic * _dhwio_st[1]] =
                                data_t<type_o>(i[ic * blksize + oc]);
                        }
...
#               pragma omp simd collapse(2)
                for (int ic = 0; ic < blksize; ++ic) {
                    for (int oc = 0; oc < blksize; ++oc) {
                        const auto dst_off = order_keep ? ic * blksize + oc :
                                                          ic * _dhwio_st[1] + oc;
                        const auto src_off = order_keep ? ic * _dhwio_st[1] + oc :
                                                          ic * blksize + oc;
                        o[dst_off] = data_t<type_o>(alpha * i[src_off]
                                     + (beta ? beta * o[dst_off] : 0));
                    }
